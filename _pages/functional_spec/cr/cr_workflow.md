...
If a booking is cancelled outside of the current consultation context, any referral document, sent as part of a booking, should be decoupled from the booking when cancelled and not be assumed a referral in its own right. This ensure ‘booking only’ services are appropritely updated.

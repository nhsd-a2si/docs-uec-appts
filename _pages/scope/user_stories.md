...
**_In order_** that I can have a discussion with the patient before choosing a service 

**_As a_** user of a 111 system or urgent care service provider  

**_I want_** to see which services accept appointments 

### Commentary
Currently the process for booking using proprietary solutions relies on a 1-1 link between the systems.  In Adastra specifically there is a database table which holds the end point for each service that has been setup for booking, using the Service ID as the Unique ID. 

When the DoS is presented the system goes to this table, sees if there is an entry for that service ID and will display an appointment button if it exists. 

To support rollout of National Standards and 111 Providers who currently use this proprietary process we will need to have a solution where they can use National Standards and proprietary solutions on the same system. To resolve this we need a way that the system can identify if a DoS Service has national standards booking, proprietary solution or no booking. 

We are recommending that a flag is set on the DoS against each Service Entry to identify if that particular service is setup to receive appointment booking, whether it be national or proprietary.   

Alongside the above we suggest an ‘endpoint type’ flag.  This will identify whether the receiving provider is setup for National Standards or Proprietary booking. (This also future proofs the system, as we can easily add addition options if new Endpoint Stores are identified) 

Also against each service we will need somewhere to store the SDS endpoint if applicable. 

To be able to control which dispositions are bookable into that service we will need to be able to manage this on the DoS.  This will require a set of checkboxes for each DX Code so that the DoS Lead/Commissioner can choose which dispositions this service accept appointments for. 

When a 111 provider’s system does a look up to the DOS, it will check if the ‘Accepts Appointments’ flag exists for each service and also check whether the disposition for that patient is ticked against the service.  The Appointment button will then be displayed as necessary.  

When they choose a service and click on the Appointment button they will then use the information from the DoS Endpoint Type to either use the local proprietary endpoint or the SDS endpoint to perform the Get Slots function. 

Using the above solution will enable Proprietary solutions to exist alongside National Standards using SDS and enable services who do not use proprietary solutions to start using National Standards.  It also gives the DoS team/commissioners control of who is currently using appointment booking and will be easier to turn off if demand or contracts require it. 

It will also be useful as a reporting tool to monitor how many DoS Services use proprietary solutions vs National Standards and to monitor the rollout and uptake of National Standards 

#### DoS Appointment booking screen mockup
![DoS mockup screen](https://github.com/nhsd-a2si/docs-uec-appts/blob/master/_pages/scope/img/DoS%20Booking%20Amendments%20Screen%20Mockup.jpg "DoS Booking Ammendment Screen")

#### DoS interaction sequence diagram
